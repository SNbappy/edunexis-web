...
import { Calendar, Users, CheckCircle2, XCircle, HelpCircle, Trash2, Pencil } from 'lucide-react'
...
function getSessionStats(session: AttendanceSessionDto) {
...
function AttendanceBadge({ pct }: { pct: number }) {
    ...
    if (pct >= 75) return <Badge variant="default">{pct}%</Badge>
    ...
    return (
        <div className="space-y-3">
            {sessions.map((session, i) => {
                const { total, present, absent, unmarked, pct } = getSessionStats(session)
                const sessionNumber = totalSessions - i
                return (
                    <motion.div
                        key={session.id}
                                ...
                                <div className="min-w-0">
                                    <div className="flex items-center gap-2 flex-wrap">
                                        <p className="text-sm font-semibold text-foreground">
                                            {'Session ' + sessionNumber + ' - ' + (session.topic || formatDate(session.date))}
                                        </p>

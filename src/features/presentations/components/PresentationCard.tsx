...
export default function PresentationCard({
    presentation: p, index = 0, onView, onDelete, onPublish, onUnpublish,
    ...
    const stateBadge = published
        ...
        : "border border-stone-200 bg-stone-50 text-stone-700 dark:border-stone-700 dark:bg-stone-900/40 dark:text-stone-300"
    const stripeClass = published ? "bg-emerald-500" : "bg-stone-400"

    const hasMenu = teacher && (onPublish || onUnpublish || onDelete)
    ...
    const handleCardClick = () => { if (onView) onView(p) }
    ...
    return (
        <motion.div
            ...
            whileHover={onView ? { y: -2 } : undefined}
            className={
                "group relative rounded-2xl border border-border bg-card shadow-[0_2px_8px_-2px_rgba(0,0,0,0.05)] transition-all hover:border-teal-200 hover:shadow-[0_12px_32px_-8px_rgba(20,184,166,0.18)] dark:hover:border-teal-800 " +
                (onView ? "cursor-pointer " : "") +
                (menuOpen ? "z-30" : "z-0")
            }
        >
            <div className={"pointer-events-none absolute bottom-3 left-0 top-3 w-[3px] rounded-full " + stripeClass} aria-hidden />
            ...
            <div className="flex items-start gap-3 px-5 py-4 pl-6">
                ...
                <div className="min-w-0 flex-1">
                                        ...
                                        {menuOpen && (
                                            ...
                                            >
                                                ...
                                                {published && onPublish && (
                                                    <MenuItem
                                                        icon={Send}
                                                        label="Republish (notify if changed)"
                                                        variant="default"
                                                        onClick={() => { onPublish(); setMenuOpen(false) }}
                                                    />
                                                )}
                    ...
                    <div className="mt-3 flex flex-wrap items-center gap-x-3 gap-y-1.5 text-[11.5px] text-muted-foreground">
                        ...
                        {!teacher && published && p.myResult?.obtainedMarks != null && (
                        ...
                        {!teacher && published && p.myResult?.isAbsent && (
